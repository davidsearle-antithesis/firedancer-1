...
handle_net_request( ctx_t             * ctx,
  ...
  if( FD_LIKELY( entry ) ) {
    switch( tag ) {
      ...
      case FD_REPAIR_KIND_ORPHAN: {
        ...
        for( uint i=0; i<FD_RSERVE_MAX_OPRHAN_SLOTS; i++ ) {
          ...
          current = current - shred->data.parent_off;
        }
        break;
      }
    }
    ...
    fd_memcpy( msg->ping.hash.uc, token, 32 );
  ...
  switch( fd_disco_shred_out_msg_type( sig ) ) {
    ...
    case FD_SHRED_OUT_MSG_TYPE_FEC: {
      ...
      for( uint i=0; i<FD_FEC_SHRED_CNT; i++ ) {
        uint j = FD_FEC_SHRED_CNT - 1U - i; /* j indexes into offsets/data forwards (0..31) */
        uint shred_idx = shred->idx - i;     /* shred_idx walks backwards from the last shred */
        ulong payload_sz    = j==0U ? offsets[ 0 ] : (offsets[ j ] - offsets[ j - 1 ]);
        uchar const * frag  = data + (j==0U ? 0U : offsets[ j - 1 ]);
        fd_shredb_insert_payload( ctx->shredb, frag, payload_sz, slot, shred_idx );
      }
      return;
    }
...
during_housekeeping( ctx_t * ctx ) {
  ...
  if( FD_UNLIKELY( now_ns-rserve->last_rotate_ts > FD_RSERVE_TOKEN_ROTATE_NS ) ) {
    ...
    rserve->token_idx++;
    fd_memcpy( rserve->token_prev, rserve->token_cur, 32UL );
    fd_rserve_derive_token( rserve->token_cur, rserve->seed, rserve->token_idx );
  }

  ping_cache_entry_t * cache = rserve->ping_cache;
  ulong slot_cnt = ping_cache_slot_cnt( cache );
  for( ulong i=0UL; i<slot_cnt; i++ ) {
    if( ping_cache_key_inval( cache[ i ].addr ) ) continue;
    if( now_ns-cache[ i ].timestamp > FD_RSERVE_PING_CACHE_TTL_NS ) {
      ping_cache_remove( cache, &cache[ i ] );
      i--; /* We need to re-check cache[ i ], since remove() backfilled it. */
    }
  }
}
