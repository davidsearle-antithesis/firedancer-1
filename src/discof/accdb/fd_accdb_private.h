...
struct __attribute__((packed)) fd_accdb_acc {
  ...
  /* offset (43 bits) and fork_id (12 bits) are packed into a
     single ulong to save 6 bytes per acc element.  Use the
     fd_accdb_acc_offset / fd_accdb_acc_fork_id / fd_accdb_acc_pack
     helpers below.  The sentinel ULONG_MAX (all bits set) means
     "no on-disk location" and is used by release and compaction. */
  ulong  offset_fork;
...
/* Pack/unpack helpers for the offset_fork field in fd_accdb_acc_t.

     bits 0..42:  offset  (43 bits, up to 8 TiB)
     bits 43..54: fork_id (12 bits, up to 4096 forks)
     bits 55..63: unused

   The "no offset" sentinel is FD_ACCDB_OFFSET_INVAL (all 43 offset
   bits set).  fork_id is always preserved independently.  Use
   fd_accdb_acc_offset_is_valid to check. */

#define FD_ACCDB_OFFSET_BITS    (43UL)
#define FD_ACCDB_OFFSET_MASK    ((1UL<<FD_ACCDB_OFFSET_BITS)-1UL)
#define FD_ACCDB_OFFSET_INVAL   FD_ACCDB_OFFSET_MASK
#define FD_ACCDB_FORKID_SHIFT   FD_ACCDB_OFFSET_BITS

static FD_FN_CONST inline ulong
fd_accdb_acc_offset( ulong offset_fork ) {
  return offset_fork & FD_ACCDB_OFFSET_MASK;
}

static FD_FN_CONST inline int
fd_accdb_acc_offset_is_valid( ulong offset_fork ) {
  return (offset_fork & FD_ACCDB_OFFSET_MASK) != FD_ACCDB_OFFSET_INVAL;
}

static FD_FN_CONST inline ushort
fd_accdb_acc_fork_id( ulong offset_fork ) {
  return (ushort)((offset_fork >> FD_ACCDB_FORKID_SHIFT) & 0xFFFUL);
}

static FD_FN_CONST inline ulong
fd_accdb_acc_pack( ulong  offset,
                   ushort fork_id ) {
  return (offset & FD_ACCDB_OFFSET_MASK)
       | ((ulong)fork_id << FD_ACCDB_FORKID_SHIFT);
}

/* Pack the "no offset" sentinel while preserving fork_id. */
static FD_FN_CONST inline ulong
fd_accdb_acc_pack_inval( ushort fork_id ) {
  return FD_ACCDB_OFFSET_INVAL
       | ((ulong)fork_id << FD_ACCDB_FORKID_SHIFT);
}
