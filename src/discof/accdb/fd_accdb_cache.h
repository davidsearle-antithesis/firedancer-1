...
/* fd_accdb_cache.h provides a static algorithm for determining, given a
   ...
   x4 geometric progression:
     ...
     Class 0: 0-128 B      (slot: 184 B)
     Class 1: 129-512 B    (slot: 568 B)
     Class 2: 513-2 KiB    (slot: 2,104 B)
     Class 3: 2K-8 KiB     (slot: 8,248 B)
     Class 4: 8K-32 KiB    (slot: 32,824 B)
     Class 5: 32K-128 KiB  (slot: 131,128 B)
     Class 6: 128K-1 MiB   (slot: 1,048,632 B)
     Class 7: 1M-10 MiB    (slot: 10,485,816 B)

   Each slot has 56 bytes of fixed metadata overhead
...
#define FD_ACCDB_CACHE_META_SZ     (56UL)
...
/* Minimum reserved slots per class.  Slots below this count are
   ...
   5-transaction bundle (130 accounts * 5 * 2 classes) can always
   execute fully in-memory. */
#define FD_ACCDB_CACHE_MIN_RESERVED (1300UL)
...
/* fd_accdb_cache_class_cnt computes the number of slots to allocate for
   ...
   Every class gets at least 1300 entries, guaranteeing a worst case
   5-transaction bundle (130 accounts * 5 transactions) can execute
   fully in memory regardless of account size mix. Returns 0 if the
   budget is too small for these minimums, or 1 on success.

   The algorithm:
   1) Reserves 650 of each class off the top.
