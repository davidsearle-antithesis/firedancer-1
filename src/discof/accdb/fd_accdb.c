...
fd_accdb_attach_child( fd_accdb_t *       accdb,
  ...
  int err;
  fd_accdb_fork_shmem_t * acquired = fork_pool_acquire( accdb->fork_shmem_pool, NULL, 1, &err );
  FD_TEST( err==FD_POOL_SUCCESS );
...
drain_deferred_frees( fd_accdb_t * accdb ) {
  if( FD_UNLIKELY( accdb->deferred_fork_head ) ) {
    ...
    fork_pool_release_chain( accdb->fork_shmem_pool, accdb->deferred_fork_head, accdb->deferred_fork_tail, 1 );
    accdb->deferred_fork_head = NULL;
    accdb->deferred_fork_tail = NULL;
  }
  ...
  acc_pool_release_chain( accdb->acc_pool_join, accdb->deferred_acc_head, accdb->deferred_acc_tail, 1 );
  accdb->deferred_acc_head = NULL;
  accdb->deferred_acc_tail = NULL;
}
...
acc_unlink( fd_accdb_t * accdb,
  ...
  if( FD_LIKELY( fd_accdb_acc_offset_is_valid( acc->offset_fork ) ) ) {
    fd_accdb_shmem_bytes_freed( accdb->shmem, fd_accdb_acc_offset( acc->offset_fork ), (ulong)acc->size+sizeof(fd_accdb_disk_meta_t) );
  ...
  if( FD_UNLIKELY( cidx!=FD_ACCDB_ACC_CIDX_INVAL ) ) {
    ...
    if( FD_LIKELY( !old_rc ) ) {
      ...
      if( FD_LIKELY( stale->key.generation==acc->key.generation &&
        ...
        ulong sc = fd_accdb_cache_class( acc->size );
...
purge_inner( fd_accdb_t *              accdb,
  ...
  if( txn!=UINT_MAX ) {
    ...
    txn_pool_release_chain( accdb->txn_pool, txn_head, txn_tail, 1 );
  }
...
background_advance_root( fd_accdb_t *       accdb,
  ...
  if( txn!=UINT_MAX ) {
    ...
    while( txn!=UINT_MAX ) {
      ...
      while( acc!=UINT_MAX ) {
        ...
        if( FD_LIKELY( (cur_acc->key.generation<=parent_fork->shmem->generation || descends_set_test( fork->descends, fd_accdb_acc_fork_id( cur_acc->offset_fork ) ) ) && !memcmp( new_acc->key.pubkey, cur_acc->key.pubkey, 32UL ) ) ) {
          uint next = cur_next;
  ...
  if( parent_txn!=UINT_MAX ) {
    ...
    txn_pool_release_chain( accdb->txn_pool, parent_head, parent_tail, 1 );
  }
...
background_compact( fd_accdb_t * accdb,
  ...
  *charge_busy = 1;
  ...
  uint acc_idx = FD_VOLATILE_CONST( accdb->acc_map[ fd_funk_rec_key_hash1( meta->pubkey, accdb->shmem->seed )&(accdb->shmem->chain_cnt-1UL) ] );
  while( acc_idx!=UINT_MAX ) {
    fd_accdb_acc_t * candidate = &accdb->acc_pool[ acc_idx ];
    uint next_idx = FD_VOLATILE_CONST( candidate->map.next );
    if( FD_LIKELY( next_idx!=UINT_MAX ) ) __builtin_prefetch( &accdb->acc_pool[ next_idx ], 0, 0 );
    if( FD_LIKELY( fd_accdb_acc_offset( candidate->offset_fork )==compact_base+compact->compaction_offset ) ) {
      acc = candidate;
      break;
    }
    acc_idx = next_idx;
  ...
  if( FD_UNLIKELY( !acc ) ) {
    ...
    /* CAS the offset from old to new.  If a concurrent release
       invalidated the offset in acc->offset_fork (dirty sentinel
       for a new commit), the CAS fails and we treat the record as
       superseded — the new data is in cache only and our relocated
       copy is stale.  fork_id is immutable for the lifetime of an
       acc node, so it stays unchanged in the packed word. */
    ulong old_offset = compact_base + compact->compaction_offset;
    ushort acc_fid = fd_accdb_acc_fork_id( acc->offset_fork );
    ulong old_packed = fd_accdb_acc_pack( old_offset, acc_fid );
    ulong new_packed = fd_accdb_acc_pack( dest_offset, acc_fid );
...
fd_accdb_acquire( fd_accdb_t *          accdb,
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    while( acc!=UINT_MAX ) {
      ...
      if( FD_LIKELY( next_acc!=UINT_MAX ) ) __builtin_prefetch( &accdb->acc_pool[ next_acc ], 0, 0 );

      if( FD_UNLIKELY( (candidate_acc->key.generation>root_generation && fd_accdb_acc_fork_id( candidate_acc->offset_fork )!=fork_id.val && !descends_set_test( fork->descends, fd_accdb_acc_fork_id( candidate_acc->offset_fork ) )) ) || memcmp( pubkeys[ i ], candidate_acc->key.pubkey, 32UL ) ) {
        acc = next_acc;
        continue;
      }
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    if( FD_LIKELY( accs[ i ] ) ) {
      if( FD_UNLIKELY( accdb->shmem->cache_class_used[ fd_accdb_cache_class( accs[ i ]->size ) ].val!=ULONG_MAX ) ) {
        requested_buckets[ fd_accdb_cache_class( accs[ i ]->size ) ]++;
      }
    }
  ...
  //   deferred until after pwritev2 completes (Step 8-9), so
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( writable[ i ] ) ) {
      for( ulong j=0UL; j<FD_ACCDB_CACHE_CLASS_CNT; j++ ) {
        ...
        ulong entry_sz = sizeof(fd_accdb_disk_meta_t) + (ulong)evicted->size;
        if( FD_LIKELY( fd_accdb_acc_offset_is_valid( evicted->offset_fork ) ) ) {
          fd_accdb_shmem_bytes_freed( accdb->shmem, fd_accdb_acc_offset( evicted->offset_fork ), entry_sz );
  ...
  //   the same batch whose acc->offset is still ULONG_MAX.  The read-
  //   iovec loop below spin-waits on offset!=ULONG_MAX, so publishing
  //   evicted offsets first prevents an intra-batch deadlock where the
  //   thread waits on an offset that only it can resolve.
  if( FD_LIKELY( batch_contiguous ) ) {
    ...
    for( int k=0; k<pending_cnt; k++ ) {
      ulong entry_sz = sizeof(fd_accdb_disk_meta_t) + (ulong)pending_accs[ k ]->size;
  ...
  FD_COMPILER_MFENCE();
  for( int k=0; k<pending_cnt; k++ ) {
    pending_accs[ k ]->offset_fork = fd_accdb_acc_pack( pending_offs[ k ], fd_accdb_acc_fork_id( pending_accs[ k ]->offset_fork ) );
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    while( FD_UNLIKELY( !fd_accdb_acc_offset_is_valid( FD_VOLATILE_CONST( accs[ i ]->offset_fork ) ) ) ) FD_SPIN_PAUSE();

    read_offsets[ read_ops_cnt ] = fd_accdb_acc_offset( accs[ i ]->offset_fork ) + sizeof(fd_accdb_disk_meta_t);
...
fd_accdb_release( fd_accdb_t *       accdb,
  ...
  for( ulong i=0UL; i<entries_cnt; i++ ) {
    ...
    fd_memcpy( target_cache_line+1UL, cache_line( accdb, 7UL, entries[ i ]._write.destination_cache_idx[ 7UL ] )+1UL, entries[ i ].data_len );
  }
  ...
  for( ulong i=0UL; i<entries_cnt; i++ ) {
    ...
    /* For overwrites, invalidate the on-disk offset BEFORE removing
       ...
       the cache will see an invalid offset and spin-wait, rather
       than reading stale on-disk bytes from the old location.  The
       atomic exchange also serializes with a concurrent compaction
       CAS (old_offset -> dest_offset). */
    ulong old_offset_fork = fd_accdb_acc_pack_inval( 0 );
    if( FD_LIKELY( entries[ i ]._overwrite ) ) {
      fd_accdb_acc_t * ow_acc = &accdb->acc_pool[ original_acc_idx ];
      old_offset_fork = FD_ATOMIC_XCHG( &ow_acc->offset_fork, fd_accdb_acc_pack_inval( fd_accdb_acc_fork_id( ow_acc->offset_fork ) ) );
      if( FD_LIKELY( fd_accdb_acc_offset_is_valid( old_offset_fork ) ) ) {
        fd_accdb_shmem_bytes_freed( accdb->shmem, fd_accdb_acc_offset( old_offset_fork ), (ulong)ow_acc->size+sizeof(fd_accdb_disk_meta_t) );
        FD_ATOMIC_FETCH_AND_SUB( &accdb->shmem->metrics->disk_used_bytes, (ulong)ow_acc->size+sizeof(fd_accdb_disk_meta_t) );
      }
    }
    ...
    if( FD_LIKELY( entries[ i ]._overwrite ) ) {
      ...
      /* The offset was already atomically swapped to ULONG_MAX and
         bytes freed above, so just update the metadata and
      ...
      fd_accdb_acc_t * acc = acc_pool_acquire( accdb->acc_pool_join, NULL, 1, NULL );
      FD_TEST( acc );
      ulong acc_idx = acc_pool_idx( accdb->acc_pool_join, acc );
      fd_memcpy( acc->key.pubkey, entries[ i ]._pubkey, 32UL );
      fd_memcpy( acc->owner, entries[ i ].owner, 32UL );
      acc->lamports       = entries[ i ].lamports;
      acc->size           = (uint)entries[ i ].data_len;
      acc->key.generation = entries[ i ]._generation;
      acc->offset_fork    = fd_accdb_acc_pack_inval( entries[ i ]._fork_id );
      ...
      /* CONCURRENCY: The cache entry is published before the acc_map
         ...
         (2) The FD_COMPILER_MFENCE above ensures stores to the acc node
             fields (pubkey, owner, lamports, size, generation, fork_id,
      ...
      fd_accdb_txn_t * txn = txn_pool_acquire( accdb->txn_pool, NULL, 1, NULL );
  ...
  for( ulong c=0UL; c<FD_ACCDB_CACHE_CLASS_CNT; c++ ) {
    ...
    for( ulong tick=0UL; tick<budget && depth+evicted<target; tick++ ) {
      ...
      if( FD_UNLIKELY( !line->persisted && acc_idx!=UINT_MAX ) ) {
        fd_accdb_acc_t * acc = &accdb->acc_pool[ acc_idx ];
        ulong entry_sz = sizeof(fd_accdb_disk_meta_t)+(ulong)acc->size;

        /* Atomically swap the offset to the invalid sentinel so that a
           concurrent compaction CAS (old_offset -> dest_offset)
        ...
        ulong old_of = FD_ATOMIC_XCHG( &acc->offset_fork, fd_accdb_acc_pack_inval( fd_accdb_acc_fork_id( acc->offset_fork ) ) );
        ulong old_offset = fd_accdb_acc_offset( old_of );
        if( FD_LIKELY( old_offset!=FD_ACCDB_OFFSET_INVAL ) ) {
        ...
        struct iovec iovs[ 2UL ] = {
          ...
          { .iov_base = (void *)(line+1UL), .iov_len = acc->size }
        };
        ...
        FD_COMPILER_MFENCE();
        acc->offset_fork = fd_accdb_acc_pack( file_off, fd_accdb_acc_fork_id( acc->offset_fork ) );
