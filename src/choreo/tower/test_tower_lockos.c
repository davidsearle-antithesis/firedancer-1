...
test_lockos( fd_wksp_t * wksp ) {
  ...
  for( ulong i = 1; i < 32; i++ ) {
    ...
    fd_tower_lockos_insert( lockos, fork_slot, &acct.vote_acc, acct.votes );
    end_intervals[i - 1] = vote_slot + (1UL << (uint)i);
  }

  for( ulong i = 0; i < 31; i++ ) {
    ulong key = lockos_interval_key( fork_slot, end_intervals[i] );
    FD_TEST( lockos_interval_map_ele_query( lockos->interval_map, &key, NULL, lockos->interval_pool ) );
  }
  FD_TEST( lockos_slot_map_ele_query( lockos->slot_map, &fork_slot, NULL, lockos->slot_pool ) );


  ulong num_keys = 0;
  for( lockos_slot_t const * slot = lockos_slot_map_ele_query_const( lockos->slot_map, &fork_slot, NULL, lockos->slot_pool );
                                      slot;
                                      slot = lockos_slot_map_ele_next_const ( slot, NULL, lockos->slot_pool ) ) {
    ulong interval_end = slot->interval_end;
    ulong key          = lockos_interval_key( fork_slot, interval_end );
    num_keys++;
