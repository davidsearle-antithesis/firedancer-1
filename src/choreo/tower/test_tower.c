...
static uchar scratch[ 1024 ] __attribute__((aligned(128)));
...
void
test_vote( void ) {
  fd_tower_t * tower = fd_tower_join( fd_tower_new( scratch, 0, 0, 0 ) );
  FD_TEST( tower );
...
test_to_vote_txn( fd_wksp_t * wksp ) {
  ...
  void *       tower_mem = fd_wksp_alloc_laddr( wksp, fd_tower_align(), fd_tower_footprint( 0, 0 ), 1UL );
  fd_tower_t * tower     = fd_tower_join( fd_tower_new( tower_mem, 0, 0, 0 ) );
...
test_switch_simple( fd_wksp_t * wksp ) {
  ...
  void * tower_mem        = fd_wksp_alloc_laddr( wksp, fd_tower_align(), fd_tower_footprint( blk_max, 0UL ), 1UL );
  void * lockos_mem       = fd_wksp_alloc_laddr( wksp, fd_tower_lockos_align(), fd_tower_lockos_footprint( blk_max, voter_max ), 1UL );
  void * stakes_mem = fd_wksp_alloc_laddr( wksp, fd_tower_stakes_align(), fd_tower_stakes_footprint( blk_max, voter_max ), 1UL );
  void * ghost_mem  = fd_wksp_alloc_laddr( wksp, fd_ghost_align(), fd_ghost_footprint( blk_max, voter_max ), 1UL );

  fd_tower_t *        tower  = fd_tower_join       ( fd_tower_new       ( tower_mem, blk_max, 0UL, 0UL            ) );
  fd_tower_lockos_t * lockos = fd_tower_lockos_join( fd_tower_lockos_new( lockos_mem, blk_max, voter_max, 0UL ) );
  fd_tower_stakes_t * stakes = fd_tower_stakes_join( fd_tower_stakes_new( stakes_mem, blk_max, voter_max, 0UL ) );
  ...
  */
  ...
  fd_tower_lockos_insert( lockos, 5, &acct.vote_acc, acct.votes );
  ...
  FD_TEST( switch_check( tower, ghost, lockos, stakes, total_stake, 5 ) == 1 );
}
...
test_switch_threshold( fd_wksp_t * wksp ) {
  ...
  void * tower_mem        = fd_wksp_alloc_laddr( wksp, fd_tower_align(), fd_tower_footprint( blk_max, 0UL ), 1UL );
  void * lockos_mem       = fd_wksp_alloc_laddr( wksp, fd_tower_lockos_align(), fd_tower_lockos_footprint( blk_max, voter_max ), 1UL );
  void * tower_stakes_mem = fd_wksp_alloc_laddr( wksp, fd_tower_stakes_align(), fd_tower_stakes_footprint( blk_max, voter_max ), 1UL );
  void * ghost_mem        = fd_wksp_alloc_laddr( wksp, fd_ghost_align(), fd_ghost_footprint( blk_max, voter_max ), 1UL );

  fd_tower_t *        tower        = fd_tower_join       ( fd_tower_new       ( tower_mem, blk_max, 0UL, 0UL) );
  fd_tower_lockos_t * lockos       = fd_tower_lockos_join( fd_tower_lockos_new( lockos_mem, blk_max, voter_max, 0UL ) );
  fd_tower_stakes_t * tower_stakes = fd_tower_stakes_join( fd_tower_stakes_new( tower_stakes_mem, blk_max, voter_max, 0UL ) );
  fd_ghost_t *        ghost        = fd_ghost_join       ( fd_ghost_new       ( ghost_mem, blk_max, voter_max, 0UL ) );
  ...
  */
  ...
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 110 ) == 0 );
  ...
  fd_tower_lockos_insert( lockos, 50, &acct.vote_acc, acct.votes );
  ...
  fd_tower_lockos_insert( lockos, 14, &acct.vote_acc, acct.votes );
  ...
  fd_tower_lockos_insert( lockos, 13, &acct.vote_acc, acct.votes );
  ...
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 110 ) == 1 );
...
test_switch_threshold_common_ancestor( fd_wksp_t * wksp ) {
  ...
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 111 ) == 0 );

  // 51, 111, 112, and 113 are all valid

  fd_tower_lockos_insert( lockos, 51, &acct.vote_acc, acct.votes );
  fd_tower_stakes_insert( tower_stakes, 51, &acct.vote_acc, acct.stake, ULONG_MAX );
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 111 ) == 1 );
  fd_tower_lockos_remove( lockos, 51 );

  fd_tower_lockos_insert( lockos, 112, &acct.vote_acc, acct.votes );
  fd_tower_stakes_insert( tower_stakes, 112, &acct.vote_acc, acct.stake, ULONG_MAX );
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 111 ) == 1 );
  fd_tower_lockos_remove( lockos, 112 );

  fd_tower_lockos_insert( lockos, 113, &acct.vote_acc, acct.votes );
...
test_tower_stakes_npow2_init( fd_wksp_t * wksp ) {
  ulong npow2_blk_maxs[] = { 50, 65, 100, 33, 17 };
  ...
  for( ulong i = 0; i < cnt; i++ ) {
    ...
    ulong footprint = fd_tower_stakes_footprint( blk_max, voter_max );
    FD_TEST( footprint );

    /* new / join */
    void * mem = fd_wksp_alloc_laddr( wksp, fd_tower_stakes_align(), footprint, 1UL );
    FD_TEST( mem );
    fd_tower_stakes_t * stakes = fd_tower_stakes_join( fd_tower_stakes_new( mem, blk_max, voter_max, 0UL ) );
    FD_TEST( stakes );
...
test_switch_eqvoc( fd_wksp_t * wksp ) {
  ...
  fd_tower_lockos_insert( lockos, 8, &acct.vote_acc, acct.votes );
  fd_tower_stakes_insert( tower_stakes, 8, &acct.vote_acc, acct.stake, ULONG_MAX );
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 7 ) == 1 );
  ...
  FD_TEST( switch_check( tower, ghost, lockos, tower_stakes, total_stake, 7 ) == 0 ); /* would fail since 8 is not a candidate anymore */

  /* add lockouts for 6', allow switching */
  fd_tower_lockos_insert( lockos, 6, &acct.vote_acc, acct.votes );
...
test_vtr_valid_join( fd_wksp_t * wksp ) {
  ...
  void *       tower_mem = fd_wksp_alloc_laddr( wksp, fd_tower_align(), fd_tower_footprint( 0, vtr_max ), 1UL );
  fd_tower_t * tower     = fd_tower_join( fd_tower_new( tower_mem, 0, vtr_max, 0 ) );
  FD_TEST( tower );
