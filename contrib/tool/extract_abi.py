...
for offset, t in offset_to_typename.items():
    ...
    if len(offset_union_info[offset])>1:
        ...
        lines.append(f"}} {c_escape(t,1)};")
        lines.append(f"FD_STATIC_ASSERT( sizeof({c_escape(t,1)})=={offset_to_type_sz[offset][0]}UL, bank_abi );")
        lines.append(f"FD_STATIC_ASSERT( alignof({c_escape(t,1)})=={offset_to_type_sz[offset][1]}UL, bank_abi );")
        blocks.append('\n'.join(lines))
        continue
    ...
    lines.append(f"}} {c_escape(t,1)};")
    for name, t_i, al, off in sorted(offset_to_struct_members[offset], key=lambda r: r[3]):
        lines.append(f"FD_STATIC_ASSERT( offsetof({c_escape(t,1)}, {c_escape(name,0)})=={off}, bank_abi );")
    lines.append(f"FD_STATIC_ASSERT( sizeof({c_escape(t,1)})=={offset_to_type_sz[offset][0]}UL, bank_abi );")
    lines.append(f"FD_STATIC_ASSERT( alignof({c_escape(t,1)})=={offset_to_type_sz[offset][1]}UL, bank_abi );")
    blocks.append('\n'.join(lines))
